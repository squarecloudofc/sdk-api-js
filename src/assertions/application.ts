...
export function assertApplication(value: unknown): asserts value is z.infer<typeof applicationSchema> {
  try {
    applicationSchema.passthrough().parse(value);
  } catch {
...
export function assertWebsiteApplication(value: unknown): asserts value is z.infer<typeof websiteApplicationSchema> {
  try {
